function gG = gfunFM(EST)

% Modified function derivative (grad F)
...
% Anulo ecuacion del nodo final para sustituirla
% G(ResALF) = 0;
...
for Ele = 1:EST.NumNod2
    ...
    % Volumen -------------------------------------------------------------
    % G(ResALF) = G(ResALF) + CD5*EST.YC*(X1+X2)*DY;
    
    gG(ResALF,VarT1) = gG(ResALF,VarT1) + CD5*EST.YC*(X1+X2)*gt1DY + CD5*EST.YC*DY*gt1X1;
    gG(ResALF,VarT2) = gG(ResALF,VarT2) + CD5*EST.YC*(X1+X2)*gt2DY + CD5*EST.YC*DY*gt2X2;
    
    % Momento y (restado)
    % G(ResALF) = G(ResALF) + CI6*(Y1*Y1 + Y1*Y2 + Y2*Y2)*DX;
    
    gG(ResALF,VarT1) = gG(ResALF,VarT1) + CI6*(Y1*Y1 + Y1*Y2 + Y2*Y2)*gt1DX + CI6*DX*(CD2*Y1*gt1Y1 + Y2*gt1Y1);
    gG(ResALF,VarT2) = gG(ResALF,VarT2) + CI6*(Y1*Y1 + Y1*Y2 + Y2*Y2)*gt2DX + CI6*DX*(Y1*gt2Y2 + CD2*Y2*gt2Y2);
